...
## 2024-05-24 - [DOM XSS via Template Literals] **Vulnerability:** Unverified URL interpolations (`${art}`) inside `innerHTML` template strings without `escapeHtml`. **Learning:** When constructing raw HTML strings with `innerHTML`, variables must be escaped; but for direct DOM assignments like `element.src`, no escaping should be used. **Prevention:** Always use `escapeHtml()` when interpolating dynamic data into `innerHTML` strings, but strictly avoid it when setting DOM properties.

## 2025-02-07 - Add input length limits to prevent DoS **Vulnerability:** Missing maximum length validation on `note`, `github`, and `message` fields in `/invite-request` and `/request-access` endpoints. **Learning:** These fields are written to unbounded KV array keys. An attacker could send massive payloads to exhaust KV size limits (25MB) and cause DoS for other users. **Prevention:** Always enforce strict string length bounds on all user inputs before serialization, and ensure existence checks (`field && field.length > X`) are performed to prevent TypeErrors when accessing `.length` on undefined fields.
## 2024-05-18 - XSS in Error Handler Suggestion
...
## 2024-05-24 - [Fix javascript: and data: XSS vulnerability in href/src] **Vulnerability:** Unsafe interpolation of dynamically generated URLs into href and src attributes allowed execution of arbitrary javascript or loading of arbitrary data if a URL started with `javascript:` or `data:`. The existing `escapeHtml` function did not prevent this because it merely escapes HTML entities and quotes, which browsers still decode inside attribute values when the protocol is unsafe. **Learning:** In frontend contexts building dynamic HTML structure where URLs are untrusted or derived from API data, HTML escaping alone does not neutralize protocol-based XSS vectors. **Prevention:** Always introduce a dedicated `getSafeUrl` function to parse the URL and explicitly reject `javascript:` and `data:` schemes before falling back to entity escaping.
## 2024-05-24 - [Fix vbscript: XSS in CodeQL] **Vulnerability:** CodeQL flagged `vbscript:` as a potential XSS vector even though it is largely obsolete in modern browsers. **Learning:** Automated security scanning tools (like CodeQL) maintain strict definitions of unsafe URL protocols and will flag omissions. **Prevention:** When writing URL sanitization functions (`getSafeUrl`), explicitly include checks for `vbscript:` alongside `javascript:` and `data:` to ensure comprehensive coverage and satisfy automated scanners.
## 2026-06-28 - [XSS] DOM-based Cross-Site Scripting via vbscript: URL spoofing
...
## 2026-09-12 - Fix Cross-Site Scripting (XSS) via innerHTML in showPromptModal
**Vulnerability:** The \`showPromptModal\` function in \`src/frontend/app.js\` concatenates the \`message\` parameter directly into a string assigned to \`modal.innerHTML\` without any escaping, leading to a potential Cross-Site Scripting (XSS) vulnerability if the \`message\` parameter is controlled by an attacker.
**Learning:** Directly concatenating unescaped inputs into HTML strings assigned to \`innerHTML\` is a common source of XSS.
**Prevention:** Always use appropriate escaping functions (like the existing \`escapeForHtml\` function) when inserting dynamic strings into HTML markup.
