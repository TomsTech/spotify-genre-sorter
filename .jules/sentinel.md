...
## 2026-09-12 - Fix XSS in Global Error Boundary **Vulnerability:** Unescaped injection of friendlyMessage into innerHTML inside the error boundary modal. **Learning:** Although friendlyMessage is derived from a hardcoded map based on error messages, unexpected characters in the mapped string or potential future changes to the map can introduce XSS when directly injected into innerHTML. **Prevention:** Always escape dynamic text before injecting it into innerHTML, even if the source is currently believed to be safe. Use existing escape utilities like escapeForHtml.
