...
## 2026-06-25 - [Security Enhancement] Robustness in HTML Escaping Functions
**Vulnerability:** The `escapeForHtml` function previously assumed its input was always a string. If passed `null`, `undefined`, or an object without a string conversion, it would throw a runtime TypeError (`text.replace is not a function`). While primarily a stability issue, in some environments, passing an unexpected object with a malicious `.replace` method could theoretically bypass escaping if not explicitly cast to a string first.
**Learning:** Custom sanitization or escaping functions must be defensive. They should validate their inputs and explicitly cast to the expected primitive type before performing string operations.
**Prevention:** When creating custom HTML escaping functions, explicitly check for `null`/`undefined` inputs and cast the input to a string (e.g., `String(text)`) before calling `.replace()`.
