...
## 2025-03-05 - [XSS] DOM-based Cross-Site Scripting via unescaped URLs in dynamically built elements
**Vulnerability:** Several URLs used directly as attributes in the application (such as `data.trackingUrl` and `changelogCache.repoUrl`) were unescaped before being concatenated into an `innerHTML` string. This creates an XSS vector if an attacker controls those URLs, as they could break out of the `href` attribute using string manipulation (e.g. `javascript:alert(1)` or `" onmouseover="alert(1)`).
**Learning:** Any URLs sourced from external APIs or backend data should be properly escaped before being concatenated to create string-based DOM content because malicious payloads can be injected via unescaped string boundaries or quotes. Even values that seem benign (like a repository URL) should be consistently escaped for defense in depth.
**Prevention:** Apply `escapeHtml` or properly sanitize all external URL fields before appending them to HTML templates that will be rendered via `innerHTML`.
## 2025-03-05 - [XSS] DOM-based Cross-Site Scripting via malicious URL schemes in dynamically built elements
**Vulnerability:** URLs used directly as attributes in the application (such as `data.trackingUrl` and `changelogCache.repoUrl`) were unescaped before being concatenated into an `innerHTML` string, and later simply escaped using `escapeHtml()`. However, `escapeHtml()` does not prevent malicious URI schemes like `javascript:` or `data:`, meaning `javascript:alert(1)` would still execute when clicked.
**Learning:** HTML entity escaping is insufficient for URLs inserted into `href` attributes. Malicious URI schemes (`javascript:`, `data:`) must be actively blocked or sanitized out before insertion.
**Prevention:** Always validate URL schemes (ensuring they start with `http://` or `https://`) or implement a robust `sanitizeUrl()` function that blocks `javascript:` and `data:` URIs *in addition to* applying `escapeHtml()`.
