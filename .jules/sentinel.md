...
## 2025-05-24 - Math.random() for Security or Critical Logic
**Vulnerability:** Found `Math.random()` used for analytics sampling decisions and retry jitter mechanisms.
**Learning:** `Math.random()` is a pseudo-random number generator and is completely predictable under certain circumstances. It should not be used in contexts involving sampling persistence, rate limits, or any quasi-security contexts.
**Prevention:** Always use `crypto.getRandomValues()` via a typed array (e.g., `Uint32Array`) and compute `array[0] / (0xFFFFFFFF + 1)` if a floating point number between 0 and 1 is required, strictly avoiding `Math.random()`.
