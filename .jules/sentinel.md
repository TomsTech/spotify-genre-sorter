...
## 2025-10-24 - [XSS] DOM-based Cross-Site Scripting in Rate Limit Banner
**Vulnerability:** The `title` and `msg` variables were being directly concatenated into the HTML string of the rate limit banner without any sanitization or escaping (`banner.innerHTML = '<strong>' + title + '</strong> — ' + msg + ...`).
**Learning:** This occurred because the rate limit banner's HTML was being constructed manually via string concatenation and then injected into the DOM via `innerHTML`, which is a pattern that can introduce XSS if the underlying string components ever originate from user input or unvalidated API payloads.
**Prevention:** Always sanitize or escape strings interpolated into HTML templates intended for the DOM. Wrapping these variables with `escapeHtml(title)` and `escapeHtml(msg)` mitigates potential XSS vulnerabilities by safely encoding dangerous characters.
