...
## 2025-06-22 - [Object Object] Coercion XSS Vulnerability
**Vulnerability:** XSS vulnerability through object-to-string coercion bypass. When injecting potentially user-controlled objects (like `error.context`) into template literals within the DOM, they were passed directly to `escapeHtml()`. If an object has a custom `toString` method, it could execute code or bypass the standard sanitization.
**Learning:** Passing raw objects to `escapeHtml()` relies on implicit coercion which can result in `[object Object]` output or security bypasses if `toString` is manipulated.
**Prevention:** Explicitly stringify objects (e.g., using `JSON.stringify()`) before passing them to sanitization functions like `escapeHtml()` to prevent coercion-based XSS vectors.
