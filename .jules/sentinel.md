...
## 2025-10-24 - [XSS] DOM-based Cross-Site Scripting in Playlist Scanner
**Vulnerability:** The `playlistName` variable in the `scanPlaylist` function was directly concatenated into the `scanningText` HTML string without any escaping, and then injected into the DOM via `innerHTML`.
**Learning:** Even if data is escaped when passed *into* an inline event handler (like `onclick="scanPlaylist('...', escapeHtml(name))"`), the browser unescapes HTML entities in the attribute before executing the JavaScript. Thus, the argument received by the function is unescaped, leading to DOM XSS if injected into `innerHTML`.
**Prevention:** Always escape user input or dynamically generated strings containing external data immediately before injecting them into `innerHTML`, regardless of how they were formatted in the calling context. Use `escapeHtml()` at the point of injection.
