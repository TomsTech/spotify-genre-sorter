...
## 2024-05-15 - Optimize array chunking in parallel KV reads
**Learning:** When processing array chunks with Promise.all(), chaining `.slice().map()` creates unnecessary intermediate arrays and increases memory allocation.
**Action:** Use `Array.from({ length: size }, (_, j) => process(arr[start + j]))` or a direct `for` loop to bypass the intermediate copy.
