...
## 2026-07-15 - Use Promise.all for parallel reads instead of sequential loop **Learning:** When processing a large list of promises derived from asynchronous fetching and mapping operations, replacing sequentially executed `.map` functions inside `await` calls with directly fetched maps combined into a single `Promise.all` can parallelize network requests and greatly reduce total latency, specifically moving from O(N) sequential requests to O(1) concurrent requests. **Action:** Next time when iterating over a list to read items individually using `await`, check if reading them using an array map alongside `Promise.all()` can optimize latency instead of causing N+1 query problem.
