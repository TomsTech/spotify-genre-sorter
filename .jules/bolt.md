...
## 2024-05-18 - PERF-031 Parallelize getAnalytics and KV listing
**Learning:** When dealing with multiple independent network/I/O requests in Cloudflare workers (such as fetching data from `kv.list` and calling external async functions like `getAnalytics` or paginating external API calls with `getArtists`), sequentially awaiting limits throughput.
**Action:** Combined `getAnalytics` and all KV listing promises into a single flat `Promise.all` call instead of nesting `Promise.all` for KV listing inside an array. For `getArtists`, we passed the full `artistIdList` without prematurely truncating via `.slice()` since `getArtists` natively implements parallelization limits in batches.
