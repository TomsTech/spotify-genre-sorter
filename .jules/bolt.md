...
## 2024-05-18 - Avoid array spread operator for large iterations **Learning:** Using the array spread operator (`...`) inside loops to concatenate elements onto an array (e.g. `results.push(...validArtists)`) is risky because JavaScript engines limit the maximum number of arguments a function can accept, throwing a "Maximum call stack size exceeded" error for large arrays. Additionally, flattening operations with spread syntax perform slowly due to intermediate allocations. **Action:** Prefer `flatMap()` combined with explicit item-by-item appending loops (e.g., `for (const item of flatMapped) { arr.push(item) }`) or use `reduce` loops over spreading arguments when concatenating dynamically sized arrays in potentially unbounded iterations.
