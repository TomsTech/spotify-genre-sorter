...
## 2023-10-25 - Use Promise.all for parallel reads instead of sequential loop
**Learning:** Even when `Promise.all` is used to replace sequential loops, running up to 1000 concurrent KV fetch operations can cause memory spikes and approach Cloudflare Worker subrequest limits if any external fetch was included.
**Action:** Always batch high-concurrency loops (e.g. 1000 keys) into smaller chunks (e.g. 50) using `Array.slice()` within a `for` loop to cap concurrency while preserving `Promise.all` benefits.
