...
## 2026-07-15 - Use Promise.all for parallel genre playlist creation API requests **Learning:** Sequential API requests within loops (e.g., `createPlaylist` and `addTracksToPlaylist`) inside chunked operations cause significant performance bottlenecks due to accumulated network latency. **Action:** Refactor sequential `for...of` loops into parallel execution using `Array.prototype.map` combined with `Promise.all()` to dispatch requests concurrently. Maintain logic safety by appropriately collecting success/failure objects instead of mutating shared arrays directly inside asynchronous blocks, and process deduplication state sets sequentially after all promises have successfully resolved.
