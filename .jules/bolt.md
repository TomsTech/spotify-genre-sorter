...
## 2025-05-28 - Optimize Promise.all parallel reads array allocation
**Learning:** Chained array methods like `.slice().map()` and `.filter()` over data sets create intermediate arrays which adds memory allocation overhead.
**Action:** Replace `.slice().map()` with `Array.from` with size bounding and native `for` loops in hot paths to bypass the intermediate array creation and optimize iteration.
