...
## 2026-09-12 - Eliminate intermediate arrays in API Route **Learning:** The original code used rest/spread syntax (...), .map(), and .forEach() to iterate over an array of prefixes and issue parallel KV lists. This created several intermediate arrays and increased garbage collection overhead, which is bad for performance, especially on frequently hit routes like API endpoints. **Action:** Replace array methods (.map, .forEach) and spread syntax with standard `for` loops to eliminate intermediate array allocations. For example, instead of `...prefixes.map(prefix => kv.list(...))`, push the promises into a pre-allocated array using a `for` loop, and use `Promise.all(listPromises)` instead of a spread.
