...
## 2024-05-27 - [Performance] Chunk concurrent network operations
**Learning:** Sequential network calls inside a loop introduce significant N+1 latency. When batching them with `Promise.all()`, we must separate sequential state mutation (deduplication) from the concurrent execution to preserve correctness while preventing subrequest limits/timeout errors. Additionally, wrapping non-blocking background tasks (like KV analytics writes) in `waitUntil` avoids delaying the client response.
**Action:** When batching operations that require stateful deduplication or validation alongside external API calls, pre-process the validation sequentially to maintain determinism, then execute the independent external API calls concurrently using `Promise.all()` inside a loop with strict chunk sizes.
