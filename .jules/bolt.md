...
## 2024-05-25 - Safely Paginating Cloudflare KV deletes
**Learning:** When clearing a large prefix from Cloudflare KV using `kv.list`, the API limits responses to 1000 items and returns a `cursor` and `list_complete` flag for pagination. Furthermore, concurrently deleting all keys returned might hit Cloudflare worker limits if not careful, though the binding is internal. Type definitions for `kv.list` in older `@cloudflare/workers-types` packages can be flawed, lacking `cursor` and incorrectly typing `list_complete`.
**Action:** When working with KV namespaces, always implement pagination when listing or operating on multiple keys, and chunk operations like `Promise.all` inside the loops into smaller batches (e.g. 50) to optimize concurrency without triggering subrequest limits. Use interface assertions to override flawed types if a package upgrade isn't immediately practical.
