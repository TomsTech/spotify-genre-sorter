...
## 2026-09-12 - PERF-015 FIX: Use chunked Promise.all for parallel reads to avoid CF worker limits **Learning:** Cloudflare Workers have a hard limit of 50 concurrent subrequests per invocation. When processing large arrays mapped to async operations like `kv.get`, intermediate array creation combined with Promise.all can cause memory and concurrency issues. Chaining `.slice().map()` creates unnecessary intermediate arrays. **Action:** Instead, use `Array.from({ length: size }, (_, j) => process(arr[start + j]))` to directly create the array of promises, avoiding the intermediate `.slice()` and `.map()` steps, and batch the operations within the worker limits.
