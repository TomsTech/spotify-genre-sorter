...
## 2024-08-10 - PERF-013: Interleave JSON.parse with KV fetches
**Learning:** Using `keys.map(async key => ...)` combined with `await Promise.all()` is a common pattern for parallel execution, but it causes intermediate objects and arrays to be created, particularly when parsing JSON and filtering results inside the async closure, resulting in additional allocations.
**Action:** By interleaving KV fetches directly into `const fetchPromises = list.keys.map(key => kv.get(key.name))` without an async wrapper, awaiting them, and then iterating sequentially with a single pass `for` loop to parse and collect non-null items, we eliminate inner closures and the `filter` method's intermediate array creation. This maintains the concurrency while effectively reducing memory allocations overhead during high-throughput requests.
## 2024-08-10 - PERF-020: Chunk parallel KV reads to avoid Cloudflare 50 subrequest limit
**Learning:** Cloudflare Workers have a hard limit of 50 concurrent subrequests per invocation. While using `Promise.all` over an array of KV keys is performant, it can easily exceed this limit if the array length is unbounded or loosely bounded to the limit size (e.g. limit: 50).
**Action:** Always chunk arrays of promises (e.g., using `slice()` in a `for` loop) into smaller batches (e.g., 10-20) when making external or KV requests, executing each batch sequentially with `await Promise.all()`. This prevents runtime limits from crashing the worker while maintaining a healthy level of concurrency.
