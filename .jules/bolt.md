...
## 2024-05-24 - [Avoid intermediate array allocations in Promise.all concurrency patterns]
**Learning:** Found several places where chunking logic for parallel processing (like `Promise.all`) used the pattern `array.slice().map()`, which allocates intermediate arrays and puts unnecessary pressure on the garbage collector. Additionally, iterating an array via index mapping (`new Array(size)` with a `for` loop) is less efficient than using `Array.from({ length })` when we can pull directly from the original source.
**Action:** Replaced instances of `slice().map()` and loop-initialized arrays with `Array.from({ length }, (_, j) => ...)` to iterate directly over the original array indices, eliminating the intermediate array creation and mapping steps, thus improving memory efficiency in high-throughput data processing paths.
