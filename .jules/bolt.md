...
## 2024-05-15 - [Batching Parallel Promises for Environment Limits]
**Learning:** When using unbounded `Promise.all` on operations in restricted environments (like Cloudflare Workers, which has a 50 subrequest limit), using `Promise.all()` over an unbounded or large array of external API fetches can lead to sudden crashes. It's important to chunk the requests while also using `async/await` rather than `.then()` continuations within the mappings to prevent unhandled promise rejection regressions on network failures.
**Action:** Instead of fetching all items at once, use sequential chunking (e.g. `batchSize = 40`) inside a `for` loop, awaiting `Promise.all(chunk)` iteratively. Always use `async/await` and inner `try/catch` within the `.map` callbacks to safely handle rejections.
