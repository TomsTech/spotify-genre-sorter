...
## 2023-10-27 - [Performance] Inefficient Nested Loop Iteration

**Learning:** When iterating over arrays of objects and extracting properties within a highly iterative or large-batch context (like chunk processing), using the `for...of` loop with object destructuring (e.g., `for (const { track } of likedTracks)`) can introduce measurable overhead due to the V8 engine creating intermediate object allocations for the destructured variables. In large datasets, these allocations increase garbage collection pressure and overall execution time. Replacing this with a traditional, non-destructuring `for` loop with cached length (e.g., `for (let i = 0, len = likedTracks.length; i < len; i++)`) allows direct property access without intermediate object allocations, improving iteration performance significantly.

**Action:** When implementing loops that run over thousands of items or in hot code paths, avoid object destructuring in the loop initializer. Instead, use a traditional `for` loop, cache the array length, and access nested properties directly via the array index to avoid unnecessary object allocations and reduce GC pressure.
