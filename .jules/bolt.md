...
## 2025-01-30 - Optimize KV Array Reads
**Learning:** Using \`.slice().map()\` in loop batches creates unnecessary intermediate arrays and allocations.
**Action:** Use \`Array.from({ length: size }, (_, j) => list[start + j])\` or standard loops to prevent intermediate array creation and optimize iteration over batches.
