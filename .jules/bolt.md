...
## 2024-05-19 - Parallelize Heterogeneous API Calls carefully with TS
**Learning:** When attempting to parallelize multiple asynchronous operations returning different structures using `Promise.all` alongside an array map operation (e.g., `[callA(), ...array.map(callB)]`), TypeScript cannot automatically infer the exact types of the destructured returned tuple due to the rest parameter.
**Action:** The solution is to separate the homogeneous parallel arrays. Await the single asynchronous call, and map the array over `Promise.all`, then await the results.
