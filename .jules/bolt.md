...
## 2025-05-28 - [Eliminating Intermediate Collections]
**Learning:** Using `Array.prototype.reduce()` in frequently executed paths or when iterating over large datasets creates unnecessary garbage collection pressure and callback function overhead, especially when combined with operations that generate intermediate arrays like `Object.values()` or spread syntax (`[...Set]`).
**Action:** Replace `reduce()` calls with native `for` or `for...of` loops that directly accumulate values in hot paths to decrease memory allocations and improve iteration performance.
