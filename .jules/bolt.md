...
## 2025-01-09 - Avoid chunk intermediate arrays **Learning:** Using `.slice().map()` in chunked processing inside frequently executed paths creates unnecessary intermediate array allocations, adding garbage collection pressure. **Action:** Replace chained array methods with a direct `for` loop to allocate only what is necessary.
