...
api.get('/admin/invites', async (c) => {
  ...
  try {
    ...
    const parsed_requests = requestsRaw ? JSON.parse(requestsRaw) as unknown : [];
    const requests = Array.isArray(parsed_requests) ? parsed_requests : [];
...
api.get('/admin/errors', async (c) => {
  ...
  try {
    ...
    const parsed_errors = errorsRaw ? JSON.parse(errorsRaw) as unknown : [];
    const errors = Array.isArray(parsed_errors) ? parsed_errors : [];
...
api.get('/admin/perf', async (c) => {
  ...
  try {
    ...
    const parsed_samples = perfRaw ? JSON.parse(perfRaw) as unknown : [];
    const samples = Array.isArray(parsed_samples) ? parsed_samples : [];
